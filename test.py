        ...
        for col in range(0, 170, int(BLOCK / 2)):
            ...
            for _ in cpc_img_color:
                if _ in pcp_show_picture_color_list:
                    point_point[str([row, col])] = point_point.get(str([row, col]), 0) + 1

    # print(point_point)
    target_x, target_y = eval(max(point_point, key=point_point.get))[0], eval(max(point_point, key=point_point.get))[1]
    try:
...
def JDcaptcha_base64(cpc_img_path_base64, pcp_show_picture_path_base64):
    ...
    :return:
    """
    return JDcaptcha(io.BytesIO(base64.b64decode(cpc_img_path_base64)),
                     io.BytesIO(base64.b64decode(pcp_show_picture_path_base64)))
...
if __name__ == '__main__':
    ...
    print("总时长{},共{}个图片, 平均时长{}每图".format(end_time - start_time, 40, (end_time - start_time) / 60))
